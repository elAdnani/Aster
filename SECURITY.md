...
The application fails closed: business APIs require an authenticated session, the first administrator can only be created from loopback, and conversations are scoped to the active profile on every server query. Passwords use salted scrypt (`N=2^17`, `r=8`, `p=1`); session identifiers are random, revocable, kept server-side and sent only through an `HttpOnly`, `SameSite=Strict` cookie. Administrative model, rule, skill-allowlist and concurrency policies are persisted and enforced by the local service; skills remain declarative until a sandboxed runtime exists.

The HTTP service validates the `Host` header before routing requests. Loopback mode accepts only `127.0.0.1`, `localhost` and `::1` on the configured port, which blocks browser DNS-rebinding requests using an attacker-controlled hostname. Non-loopback deployments must explicitly list public hostnames in `ASTER_ALLOWED_HOSTS`; this is an additional boundary, not a replacement for TLS or authenticated remote sessions.
...
Returning to the profile chooser clears the selected profile on the trusted service, rotates the session cookie and immediately removes conversations, projects, tasks, attachments and active generation state from the browser DOM and memory. Logging out performs the same browser purge before invalidating the account session. This reduces accidental disclosure on a shared screen; it does not protect against another person controlling the same operating-system account or browser profile.
